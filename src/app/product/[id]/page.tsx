      ...
      <nav className="sticky top-0 z-50 backdrop-blur-xl" style={{ borderBottom: '1px solid var(--border)', background: 'color-mix(in srgb, var(--bg) 85%, transparent)' }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            {user ? (
              <button onClick={logout} className="text-xs px-3 py-2 rounded-lg" style={{ color: 'var(--text-muted)' }}>Sign out</button>
            ) : (
              <Link href="/auth/login" className="text-xs font-medium px-3 py-2 rounded-lg" style={{ color: 'var(--text-secondary)', border: '1px solid var(--border)' }}>Sign In</Link>
            )}
          </div>
        </div>
      </nav>
            ...
            <div className="space-y-8 lg:pt-8">
              ...
              <div>
                <p className="text-xs tracking-wider uppercase font-medium mb-4" style={{ color: 'var(--text-muted)' }}>
                  Color — <span className="font-semibold" style={{ color: 'var(--text)' }}>{selectedColor.name}</span>
                </p>
                <div className="flex gap-3">
                    ...
                    <button key={c.name} onClick={() => setSelectedColor(c)}
                      ...
                      {selectedColor.name === c.name && (
                        <span className="absolute inset-0 flex items-center justify-center">
                          <Check size={16} className={c.hex === '#ffffff' || c.hex === '#f5f0e8' || c.hex === '#f5f0e8' ? 'text-black' : 'text-white'} />
                        </span>
                      )}
                    </button>
                  ))}
              ...
              <div>
                <div className="flex items-center justify-between mb-4">
                  <p className="text-xs tracking-wider uppercase font-medium" style={{ color: 'var(--text-muted)' }}>
                    Size — <span className="font-semibold" style={{ color: 'var(--text)' }}>{selectedSize}</span>
                  </p>
              ...
              {/* Quantity */}
              <div>
                <p className="text-xs tracking-wider uppercase font-medium mb-4" style={{ color: 'var(--text-muted)' }}>Quantity</p>
              ...
              <div className="space-y-3 pt-4">
                <button onClick={handleAdd}
                  ...
                  {added ? (
                    <><Check size={20} /> Added to Bag — ${(product.price * qty).toFixed(2)}</>
                  ) : (
                    <><ShoppingBag size={20} /> Add to Bag — ${(product.price * qty).toFixed(2)}</>
                  )}
                </button>
                <Link href="/"
